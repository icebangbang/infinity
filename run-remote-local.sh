## 在目标服务器中执行用
# bash run-remote-local.sh infinity
...
#docker run --privileged=true  -e "index=1"  -e "FLASK_ENV=infinity" --name=dao-celery-schedule-1  --net=host --rm -it dao-celery-schedule:latest
