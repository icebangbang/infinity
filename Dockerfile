...
RUN pip3 install --trusted-host mirrors.cloud.aliyuncs.com --user -i https://mirrors.cloud.aliyuncs.com/pypi/simple --no-cache-dir -r requirements.txt
...
CMD  /usr/local/bin/gunicorn/gunicorn -c app/gunicorn/gun.py --log-config app/gunicorn/gunicorn_logging.conf -b 0.0.0.0:${port} manage:app -e FLASK_ENV=${profiles}
